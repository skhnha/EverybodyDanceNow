...
if len(os.listdir('./data/source/images'))<100:
    ...
    while (cap.isOpened()):
        flag, frame = cap.read()
        if flag == False or i == f_end + f_start:
            break
        if i >= f_start:
            cv2.imwrite(str(img_dir.joinpath('{:05}.png'.format(i-f_start))), frame)
        if (i-f_start)%100 == 0:
            print('Has generated %d picetures'%(i-f_start))
        i += 1
...
from net.rtpose_vgg import get_model
# utils
from openpose_utils import remove_noise, get_pose


weight_name = './src/PoseEstimation/net/weight/pose_model.pth'
